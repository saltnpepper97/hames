...
to three suggested answers with optional multiline descriptions plus an
always-available one-line custom response, then return the answer to that same
run.
...
choose it. Press `N` (or click `N add note`) to attach a one-line note to that
answer. `Write something else` is a separate final radio choice that opens a
one-line custom answer.
